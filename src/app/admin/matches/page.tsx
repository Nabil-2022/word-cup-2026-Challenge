    ...
    <AdminShell title="Matches">
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
        <table className="w-full min-w-[860px] text-left text-sm">
          ...
          <tbody>{matches.map((match) => <tr key={match.id} className="border-t border-slate-200"><td className="p-3 font-mono text-xs">{match.id}</td><td>{match.groupName}</td><td>{match.team1} vs {match.team2}</td><td>{match.matchDate.toISOString()}</td><td>{match.scoreTeam1 ?? "-"} / {match.scoreTeam2 ?? "-"}</td><td>{match.result ?? "-"}</td><td>{match.status}</td></tr>)}</tbody>
        </table>
      </div>
    </AdminShell>
  );
